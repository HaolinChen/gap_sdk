...
class MatchInsertCopies(Matcher):
    ...
    def match(self, G: GraphView, set_identity: bool = True, **kwargs):
        ...
        for node in split_nodes:
            has_modified_graph = self.find_direct_connects(G, node, has_modified_graph)
        concat_nodes = [node for node in G.nodes(node_classes=ConcatParameters)]
        for node in concat_nodes:
            has_modified_graph = self.find_direct_connects(G, node, has_modified_graph, find_output=False)
