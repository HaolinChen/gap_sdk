...
class MatchRnnUnpack(Matcher):
    ...
    def find_unpack(self, G, node, first=True, out_shape=None, in_idx=None):
        if isinstance(node, RNNBaseParameters):
            ...
            before = sum(dim.shape[node.axis] for dim in node.in_dims[:in_idx:])
            ...
            if node.shape[0] == node.old_shape[0] * node.old_shape[1]:
                ...
                return []
            d_axis = next(iter([idx for idx, elem in enumerate(out_shape) if elem[0] == 'd']))
                    ...
                    new_shape.append(
                        (
                            ...
                            (
                                ...
                                tuple([in_shape[1][0] * d_idx for d_idx in out_shape[d_axis][1][1]])
                            )
                        )
                    )
                else:
    ...
    def validate_slices(self, G, rnn_nodes):
        ...
        for rnn_unpack in rnn_nodes:
            ...
            time_axis = next(iter([idx for idx, elem in enumerate(unpack_dim) if elem[0] == 't']))
            if isinstance(unpack_node, StridedSliceParameters):
                if not unpack_node.only_slices(time_axis):
                    LOG.debug("can't remove %s. Slice operates on non timestep axis", unpack_node.name)
    ...
    def process_path(self, G, rnn_unpack, modified_nodes):
        ...
        for node, node_dim in rnn_unpack[:-1:]:
            if node in modified_nodes:
                ...
                LOG.info("%s", f"setting {node.name} to single timestep output")
                ...
                LOG.info("%s", f"passing concat {node.name} on axis {node.axis}")
            elif isinstance(node, ReshapeParameters):
                # divide the shape of the time axis by the original number of timesteps
                in_t_axis = next(iter([idx for idx, dim in enumerate(last_dim) if dim[0] == 't']))
                in_shape = node.old_shape.shape
                out_t_axis = next(iter([idx for idx, dim in enumerate(node_dim) if dim[0] == 't']))
                ...
                if not node.transpose_in or not node.transpose_in[0]:
                    ...
                    t_axis = next(iter([idx for idx, dim in enumerate(node_dim) if dim[0] == 't']))
                    new_transpose = [
                        axis - 1 if axis > t_axis else axis for axis in node.transpose_in[0] if axis != t_axis]
                    if new_transpose == sorted(new_transpose):
                        remove_node = True
                    else:
                        LOG.info("%s", f"setting {node.name} transpose to {new_transpose}")
    ...
    def match(self, G: GraphView, set_identity: bool = True, **kwargs):
        ...
        for unpack_node, rnn_unpacks in rnn_nodes_by_slice.items():
            ...
            assert len(in_edges) == 1, "expecting unpack node to have only one in edge"
            in_edge = in_edges[0]
            ...
            if changes_shape:
                ...
                if G.quantization:
                    G.quantization[NodeId(reshape)] = G.quantization[NodeId(unpack)]
            else:
                for out_edge in out_edges:
                    G.add_edge(NNEdge(from_node=in_edge.from_node, to_node=out_edge.to_node,
                                      from_idx=in_edge.from_idx, to_idx=out_edge.to_idx))
