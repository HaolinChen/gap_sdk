...
#include "CNN_BasicKernels_HWC_SQ8.h"
...
*/
void static __attribute__ ((noinline)) KerReductIO_Compact_HWC_SQ8(int *__restrict__ In, unsigned int Size, unsigned int CoreId, unsigned int ChunkCell)
...

{
	...
	int W = Arg->W;
	int H = Arg->H;
	unsigned int CoreId = gap_coreid(), ChunkCell = ChunkSize(W), First = CoreId*ChunkCell, Last  = Min(First+ChunkCell, W);
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
		                int Acc0 = gap_clip(In[h*W*Feat + w*Feat + 2*c+0], 7);
		                int Acc1 = gap_clip(In[h*W*Feat + w*Feat + 2*c+1], 7);

		                Out[h*W*Feat + w*Feat + 2*c  ] = Acc0; 
		                Out[h*W*Feat + w*Feat + 2*c+1] = Acc1;
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clip(In[h*W*Feat + w*Feat + Feat-1], 7);
		                Out[Feat-1] = Acc0;
		        }
		}
		gap_waitbarrier(0);
...

{
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
		                int Acc0 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + 2*c+0], Scale[c],   ScaleN[c  ]), 7);
		                int Acc1 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + 2*c+1], Scale[c+1], ScaleN[c+1]), 7);

		                Out[h*W*Feat + w*Feat + 2*c  ] = Acc0; 
		                Out[h*W*Feat + w*Feat + 2*c+1] = Acc1;
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + Feat-1], Scale[Feat-1], ScaleN[Feat-1]), 7);
		                Out[Feat-1] = Acc0;
		        }
		}
		gap_waitbarrier(0);
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
		                ...
		                Out[h*W*Feat + w*Feat + 2*c  ] = Max(0, Acc0); 
		                Out[h*W*Feat + w*Feat + 2*c+1] = Max(0, Acc1);
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + Feat-1], Scale[Feat-1], ScaleN[Feat-1]), 7);
		                Out[Feat-1] = Max(0, Acc0);
		        }
		}
		gap_waitbarrier(0);
	}
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
		                ...
		                Out[h*W*Feat + w*Feat + 2*c  ] = AT_CLIP_POS(Acc0, A0);
		                Out[h*W*Feat + w*Feat + 2*c+1] = AT_CLIP_POS(Acc1, A0);
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + Feat-1], Scale[Feat-1], ScaleN[Feat-1]), 7);
		                Out[Feat-1] = AT_CLIP_POS(Acc0, A0);
		        }
		}
		gap_waitbarrier(0);
	}
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
				...
				Out[h*W*Feat + w*Feat + 2*c  ] = gap_clip(AT_SCALE(AT_CLIP_POS(Acc0 + B0, A0) * C0, ActScale, ActScaleN), 7);
				Out[h*W*Feat + w*Feat + 2*c+1] = gap_clip(AT_SCALE(AT_CLIP_POS(Acc1 + B0, A0) * C0, ActScale, ActScaleN), 7);
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + Feat-1], Scale[Feat-1], ScaleN[Feat-1]), 7);
		                Out[Feat-1] = gap_clip(AT_SCALE(AT_CLIP_POS(Acc0 + B0, A0) * C0, ActScale, ActScaleN), 7);
		        }
		}
		gap_waitbarrier(0);
	}
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
				...
				Out[h*W*Feat + w*Feat + 2*c  ] = gap_clip(AT_SCALE(AT_CLIP_POS(Acc0 + B0, A0) * C0 * Acc0, ActScale, ActScaleN), 7);
				Out[h*W*Feat + w*Feat + 2*c+1] = gap_clip(AT_SCALE(AT_CLIP_POS(Acc1 + B0, A0) * C0 * Acc1, ActScale, ActScaleN), 7);
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + Feat-1], Scale[Feat-1], ScaleN[Feat-1]), 7);
		                Out[Feat-1] = gap_clip(AT_SCALE(AT_CLIP_POS(Acc0 + B0, A0) * C0 * Acc0, ActScale, ActScaleN), 7);
		        }
		}
		gap_waitbarrier(0);
	}
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
				...
				Out[h*W*Feat + w*Feat + 2*c  ] = gap_clip(AT_SCALE((Neg0*Acc0N+Pos0*Acc0), ActScale, ActScaleN), 7);
				Out[h*W*Feat + w*Feat + 2*c+1] = gap_clip(AT_SCALE((Neg1*Acc1N+Pos1*Acc1), ActScale, ActScaleN), 7);
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clip(AT_SCALE(In[h*W*Feat + w*Feat + Feat-1], Scale[Feat-1], ScaleN[Feat-1]), 7);
				int Neg0 = gap_bitextractu(Acc0, 1, 31), Pos0 = !Neg0;
				int Acc0N = AT_NORM(Acc0 * A0, 7);
				Out[Feat-1] = gap_clip(AT_SCALE((Neg0*Acc0N+Pos0*Acc0), ActScale, ActScaleN), 7);
		        }
		}
		gap_waitbarrier(0);
	}
...

{
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
		                Out[h*W*Feat + w*Feat + 2*c  ] = gap_clipu(In[h*W*Feat + w*Feat + 2*c+0], 8);
		                Out[h*W*Feat + w*Feat + 2*c+1] = gap_clipu(In[h*W*Feat + w*Feat + 2*c+1], 8);
		        }
		        if (Feat&0x1) {
		                Out[Feat-1] = gap_clipu(In[h*W*Feat + w*Feat + Feat-1], 8);
		        }
		}
		gap_waitbarrier(0);
	}
...

{
	...
	for (int h=0; h<H; h++) {
		for (int w=First; w<Last; w++) {
			for (int c=0; c<Feat/2; c++) {
		                //int Acc0 = gap_clipu(AT_SCALE(In[h*W*Feat + w*Feat + 2*c+0], Scale[c],   ScaleN[c  ]), 8);
		                //int Acc1 = gap_clipu(AT_SCALE(In[h*W*Feat + w*Feat + 2*c+1], Scale[c+1], ScaleN[c+1]), 8);
						int Acc0 = gap_clipu(In[h*W*Feat + w*Feat + 2*c+0], 8);
						int Acc1 = gap_clipu(In[h*W*Feat + w*Feat + 2*c+1], 8);

		                Out[h*W*Feat + w*Feat + 2*c  ] = AT_CLIP_POS(Acc0, A0);
		                Out[h*W*Feat + w*Feat + 2*c+1] = AT_CLIP_POS(Acc1, A0);
		        }
		        if (Feat&0x1) {
		                int Acc0 = gap_clipu(In[h*W*Feat + w*Feat + Feat-1], 8);
		                Out[Feat-1] = AT_CLIP_POS(Acc0, A0);
		        }
		}
		gap_waitbarrier(0);
...
//         for (int h=0; h<H; h++){
//             for (int w=0; w<W; w++){
//                 printf("%3u ", Out[h*W*Feat + w*Feat + c]);
//             }
//             printf("\n");
//         }
//         printf("--- chan %d\n", c);
