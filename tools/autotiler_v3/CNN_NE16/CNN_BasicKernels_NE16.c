...
void KerConv3x3Stride1_DxDy_NE16(KerConv_NE16_T *Arg)
{
	...
	int PadL = Arg->Pad[0], PadT = Arg->Pad[1], PadR = Arg->Pad[2], PadB = Arg->Pad[3];
	int Nb_WO = 1;
	int Nb_HO = 1;
		...
		Last_Rem_HI = Last_Rem_HO?(Last_Rem_HO+2):0;
			...
			Last_Rem_WI = Last_Rem_WO?(Last_Rem_WO+2):0;
        		for (int ho=0; ho<Nb_HO_Subtiles; ho++){
					...
					SetNE16_InPointer (In  + Tile_InFeat*(Dx*3*wo+i) + Tile_InFeat*Tile_InW*(Dy*3*ho+j));
...
void KerConv3x3Stride2_NE16(KerConv_NE16_T *Arg)
{
	...
	SetNE16_InPointer (In  +          4*Tile_InW  * Tile_InFeat  * subtile_i_major   +          4*Tile_InFeat  * subtile_j_major);
	...
	SetNE16_ConfigPad     ((v4s) {PadL, 0, PadT, 0}, Arg->Pad_Val);
	SetNE16_ConfigFMask   ((v4s) {0, 0, 0, 0});
	SetNE16_WOffset       (Arg->W_Offset);
	SetNE16_GenConfig     (Gen_Cfg);
...
/* Software emulated stride: one spatial pixel output computed at the time (Nb_Ho/Wo=1, Rem_Ho/Wo=1 --> Nb_Hi/Wi=1, Rem_Hi/Wi=3)
   the stride is emulated by moving the input pointer of the stride at each iteration */
void KerConv3x3StrideSxxSy_NE16(KerConv_NE16_T *Arg)
{
	...
	int Nb_HO	= 1;
	int Rem_WI	= 3;
	int Rem_HI	= 3; // Only one 3x3 input subtile at the time --> slow inference
	int OutBytes	= Arg->Mode16?2:1;
        int H_subtiles  = Arg->Tile_OutH;
        int W_subtiles  = Arg->Tile_OutW;
	...
	SetNE16_Strides       (Arg->Tile_InFeat, Arg->Tile_InFeat * Arg->Tile_InW, 0, 				// In_D0, In_D1, In_D2 - unused
			       32, OutBytes * Arg->Tile_OutFeat, OutBytes * Arg->Tile_OutFeat * Arg->Tile_OutW,	// Out_D0, Out_D1, Out_D2
			       2*3*3, 2*3*3*Arg->Qw, 0);				                        // Weights_D0, Weights_D1, Weights_D2
	SetNE16_Reminders     (Rem_WI, Rem_HI, Rem_KI, Rem_KO, Rem_WO, Rem_HO);
	SetNE16_Dim           (Nb_KI, Nb_KO, Nb_WO, Nb_HO);
	SetNE16_ConfigPad     (Arg->Pad, Arg->Pad_Val);
	SetNE16_ConfigFMask   ((v4s) {0, 0, 0, 0});
	...
	// update input / output pointers
	SetNE16_InPointer (Arg->In  + Sx*Arg->Tile_InW * Arg->Tile_InFeat  * subtile_i_major + Sx*Arg->Tile_InFeat * subtile_j_major);
	SetNE16_OutPointer(Arg->Out + 1*Arg->Tile_OutW * Arg->Tile_OutFeat * subtile_i_major + 1*Arg->Tile_OutFeat * subtile_j_major);
	subtile_j_major ++;
	...
	do {
		...
		// update indeces
		subtile_j_major ++;
		if(subtile_j_major==(W_subtiles+1)) {
			subtile_j_major = 0;
			subtile_i_major ++;
		}

	} while(subtile_i_major<(H_subtiles+1) && subtile_j_major<(W_subtiles+1));
...
void KerConv5x5Stride1_NE16(KerConv_NE16_T *Arg)
{
	...
	int Rem_WI	= 0;
	int Rem_HI	= 0;
        int W_Stride1   = 2*3*3*Arg->Qw;
        unsigned int bias_reset_mask, norm_shift_reset_mask, Gen_Cfg;
        unsigned int Default_cfg = Arg->Default_NE16_Job_Cfg;
	int Streamout   = (Default_cfg >> NE16_SHIFT_OUTQUANT) & NE16_MASK_OUTQUANT;
        int OutBytes	= Streamout?(Arg->Mode16?2:1):4;
        int Out_Stride0 = Streamout?(Arg->Mode16?16:0):32;
	...
	volatile int job_id;
        NE16_SETPRIORITY_NE16(); // priority to NE16 w.r.t. cores, DMA
	// Divide the 5x5 filter into 4 parts (overall cover 7x7 input subtile and compute 3x3 output subtile):
	//   - 3x3 FMask={0,0,0,0}
	//   - 3x3 FMask={0,1,0,0}
	//   - 3x3 FMask={0,0,1,0}
	//   - 3x3 FMask={0,0,1,0}
	...
	// preload all common configuration for the layer in the first job
       	// update input / output pointers
	SetNE16_InPointer (Arg->In  + 3*Arg->Tile_InW  * Arg->Tile_InFeat  * (subtile_i_major+subfilter_i) + 3*Arg->Tile_InFeat  * (subtile_j_major+subfilter_j));
	SetNE16_OutPointer(Arg->Out + 3*Arg->Tile_OutW * Arg->Tile_OutFeat * (subtile_i_major) + 3*Arg->Tile_OutFeat * (subtile_j_major));
	SetNE16_WeightsPointer(Arg->Filter + 3*5*Arg->Tile_InFeat*subfilter_i + 3*Arg->Tile_InFeat*subfilter_j);
	SetNE16_BiasPointer   (Arg->Bias);
	SetNE16_ScalePointer  (Arg->Scale);
	SetNE16_ScaleNPointer (Arg->ScaleN);
	SetNE16_Strides       (Arg->Tile_InFeat, Arg->Tile_InFeat * Arg->Tile_InW, 0, 	        		// In_D0, In_D1, In_D2 - unused
		      	       32, OutBytes * Arg->Tile_OutFeat, OutBytes * Arg->Tile_OutFeat * Arg->Tile_OutW,	// Out_D0, Out_D1, Out_D2 div 2 to take into account strideness
		      	       2*5*5, 2*5*5*Arg->Qw, 0);                                			// Weights_D0, Weights_D1, Weights_D2
	SetNE16_Reminders     (Rem_WI, Rem_HI, Rem_KI, Rem_KO, Rem_WO, Rem_HO);
	SetNE16_Dim           (Nb_KI, Nb_KO, Nb_WO, Nb_HO);
	SetNE16_ConfigPad     (Arg->Pad, Arg->Pad_Val);
	SetNE16_ConfigFMask   ((v4s) {0, 0, 0, 0});
	...
	SetNE16_Dim           (Nb_KI, Nb_KO, Nb_WO, Nb_HO);
	SetNE16_ConfigPad     (Arg->Pad, Arg->Pad_Val);
	SetNE16_ConfigFMask   ((v4s) {0, subfilter_j, 0, subfilter_i});
	subfilter_j++;
	// next row of subfilters
	subfilter_j = 0;
	subfilter_i = 1;
	...
	do {
		...
		// update input / output pointers
		if(subtile_i_major==H_subtiles && subtile_j_major==W_subtiles) {
			Rem_HO = Arg->Tile_OutH % 3;  Rem_WO = Arg->Tile_OutW % 3;
			Rem_HI = Rem_HO?(Rem_HO+2):0; Rem_WI = Rem_WO?(Rem_WO+2):0;
			SetNE16_Reminders(Rem_WI, Rem_HI, Rem_KI, Rem_KO, Rem_WO, Rem_HO);
		}
		else if(subtile_i_major==H_subtiles) {
			Rem_HO = Arg->Tile_OutH % 3;  Rem_WO = 0;
			Rem_HI = Rem_HO?(Rem_HO+2):0; Rem_WI = 0;
			SetNE16_Reminders(Rem_WI, Rem_HI, Rem_KI, Rem_KO, Rem_WO, Rem_HO);
		}
		else if(subtile_j_major==W_subtiles) {
			Rem_HO = 0; Rem_WO = Arg->Tile_OutW % 3;
			Rem_HI = 0; Rem_WI = Rem_WO?(Rem_WO+2):0;
			SetNE16_Reminders(Rem_WI, Rem_HI, Rem_KI, Rem_KO, Rem_WO, Rem_HO);
		}
		else {
		...
		SetNE16_InPointer (Arg->In  + 3*Arg->Tile_InW  * Arg->Tile_InFeat  * (subtile_i_major+subfilter_i) + 3*Arg->Tile_InFeat  * (subtile_j_major+subfilter_j));
		SetNE16_OutPointer(Arg->Out + 3*Arg->Tile_OutW * Arg->Tile_OutFeat * (subtile_i_major) + 3*Arg->Tile_OutFeat * (subtile_j_major));
		SetNE16_WeightsPointer(Arg->Filter + 3*5*Arg->Tile_InFeat*subfilter_i + 3*Arg->Tile_InFeat*subfilter_j);	// Weights_D0, Weights_D1, Weights_D2
		SetNE16_ConfigPad     (Arg->Pad, Arg->Pad_Val);
		SetNE16_ConfigFMask   ((v4s) {0, subfilter_j, 0, subfilter_i});
		...
		if (subfilter_i == 2 && subfilter_j == 2){
			subtile_j_major ++;
			if(subtile_j_major==(W_subtiles+1)) {
				subtile_j_major = 0;
				subtile_i_major ++;
	...
	} while(subtile_i_major<(H_subtiles+1) && subtile_j_major<(W_subtiles+1) && subfilter_j<4 && subfilter_i<4);
